...
std::string IpCamCentral::handleCliCommand(std::string command)
{
	try
	{
		...
		if(command.compare(0, 12, "peers create") == 0 || command.compare(0, 2, "pc") == 0)
		{
			...
			{
				std::shared_ptr<IpCamPeer> peer = createPeer(BaseLib::Systems::LogicalDeviceType(IPCAM_FAMILY_ID, deviceType), serialNumber, false);
...
std::shared_ptr<IpCamPeer> IpCamCentral::createPeer(BaseLib::Systems::LogicalDeviceType deviceType, std::string serialNumber, bool save)
{
	try
	{
...
PVariable IpCamCentral::getDeviceInfo(BaseLib::PRpcClientInfo clientInfo, uint64_t id, std::map<std::string, bool> fields)
{
	try
	{
		if(id > 0)
		...
		else
		{
			...
			std::vector<std::shared_ptr<IpCamPeer>> peers;
			//Copy all peers first, because listDevices takes very long and we don't want to lock _peersMutex too long
			_peersMutex.lock();
			for(std::map<uint64_t, std::shared_ptr<BaseLib::Systems::Peer>>::iterator i = _peersById.begin(); i != _peersById.end(); ++i)
			{
				peers.push_back(std::dynamic_pointer_cast<IpCamPeer>(i->second));
			}
			_peersMutex.unlock();

			for(std::vector<std::shared_ptr<IpCamPeer>>::iterator i = peers.begin(); i != peers.end(); ++i)
			{
...
PVariable IpCamCentral::putParamset(BaseLib::PRpcClientInfo clientInfo, std::string serialNumber, int32_t channel, ParameterGroup::Type::Enum type, std::string remoteSerialNumber, int32_t remoteChannel, PVariable paramset)
{
	try
	{
		std::shared_ptr<IpCamPeer> peer(getPeer(serialNumber));
		...
		{
			std::shared_ptr<IpCamPeer> remotePeer(getPeer(remoteSerialNumber));
...
PVariable IpCamCentral::putParamset(BaseLib::PRpcClientInfo clientInfo, uint64_t peerID, int32_t channel, ParameterGroup::Type::Enum type, uint64_t remoteID, int32_t remoteChannel, PVariable paramset)
{
	try
	{
		std::shared_ptr<IpCamPeer> peer(getPeer(peerID));
