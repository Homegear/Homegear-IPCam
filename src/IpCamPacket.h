...
class IpCamPacket : public BaseLib::Systems::Packet
{
    public:
        IpCamPacket();
        IpCamPacket(std::string& baseUrl, std::string& path, std::string& function, std::string& username, std::string& password, std::shared_ptr<std::vector<std::pair<std::string, std::string>>> valuesToSet);
        virtual ~IpCamPacket();
